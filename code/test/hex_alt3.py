...
def hamiltonian(p, M, k_val):
    ...
    def A(nphi_val, m_val):
        value = 2 * np.cos(2 * np.pi * nphi_val * (m_val + 1/2)/3)
        return value

    def D(nphi_val, m_val):
        value = 2 * np.cos(np.pi * nphi_val * (m_val + 1/2)/3) * np.exp(- 1j * np.pi * nphi * 0.5 * (2 * (m_val/3) + 1))
        return value

    upper_diag_array = np.array([A(nphi, m) for m in range(q)])
    Hamiltonian += np.roll(np.diag(upper_diag_array), 0, axis=1)

    upper_diag_array2 = np.array([D(nphi, m) for m in range(q)])
    Hamiltonian += np.roll(np.diag(upper_diag_array2), 3, axis=1)
    Hamiltonian += np.roll(np.diag(np.conj(upper_diag_array2)), 3, axis=0)
